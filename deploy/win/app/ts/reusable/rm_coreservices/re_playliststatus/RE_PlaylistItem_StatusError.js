...
(function (rm_coreservices) {
    var RE_PlaylistItem_StatusError = (function (_super) {
        ...
        RE_PlaylistItem_StatusError.prototype.notifyChildError = function (event, eventQueue, error, aPlaylistController, aRenderingController, context, callback) {
            if (this.getDiagram().getOwner().getParent() == null)
                return;
            this.postEvent(amPlaylistItemEventType.am_coreservices.AE_PlaylistItemEventType.EVT_notifyChildError, this.getDiagram().getOwner().getParent().getLogic(), null, this.getDiagram().getOwnerIdxAsChild(), eventQueue, error, context, callback);
        };
