var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
define(["require", "exports", "../../../../../app/ts/reusable/rm_general/r_entity/R_Entity"], function (require, exports, rmGeneralEntity) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var rm_renderingservices;
    (function (rm_renderingservices) {
        var RE_HtmlRendererPool = /** @class */ (function (_super) {
            __extends(RE_HtmlRendererPool, _super);
            //---------------------------
            function RE_HtmlRendererPool() {
                var _this = _super.call(this) || this;
                _this._htmlRendererList = new Array();
                return _this;
            }
            //--------------------------------------------
            RE_HtmlRendererPool.prototype.initPool = function (nbHtmlRenderers, error) {
                var i = 0;
                var newHtmlRenderer = null;
                var errorNo = 1;
                for (i = 0; i < nbHtmlRenderers; i++) {
                    newHtmlRenderer = this._aSrvLocator._iServiceCreation.createDefaultService_HtmlRenderer(this._aSrvContainer, this._aSrvLocator, this._aSrvLog, i, error);
                    if (error.isError()) {
                        errorNo = 1;
                        break;
                    }
                    newHtmlRenderer._iService.injectDependencies(this._aSrvContainer, this._aSrvLocator, this._aSrvLog, error);
                    this.addHtmlRenderer(newHtmlRenderer);
                }
                return errorNo;
            };
            //--------------------------
            RE_HtmlRendererPool.prototype.addHtmlRenderer = function (htmlRenderer) {
                this._htmlRendererList.push(htmlRenderer);
            };
            //--------------------------
            RE_HtmlRendererPool.prototype.reset = function () {
                var i = 0;
                for (i = 0; i < this._htmlRendererList.length; i++) {
                    this._htmlRendererList[i].reset();
                }
            };
            //------------
            RE_HtmlRendererPool.prototype.copy = function (src) {
                if (src == null)
                    return 1;
                return 0;
            };
            //-----------------------------
            RE_HtmlRendererPool.prototype.getHtmlRendererList = function () {
                return this._htmlRendererList;
            };
            //--------------------------
            RE_HtmlRendererPool.prototype.getFreeHtmlRenderer = function () {
                var freeRenderParams = null;
                var i = 0;
                for (i = 0; i < this._htmlRendererList.length; i++) {
                    if (this._htmlRendererList[i] == null)
                        continue;
                    if (!this._htmlRendererList[i].isFree())
                        continue;
                    if (!this._htmlRendererList[i].setIsFree(false))
                        continue;
                    freeRenderParams = this._htmlRendererList[i];
                    break;
                }
                return freeRenderParams;
            };
            //--------------------------
            RE_HtmlRendererPool.prototype.releaseHtmlRenderer = function (htmlRenderer) {
                if (htmlRenderer == null)
                    return false;
                htmlRenderer.setIsFree(false);
                return true;
            };
            return RE_HtmlRendererPool;
        }(rmGeneralEntity.rm_general.R_Entity));
        rm_renderingservices.RE_HtmlRendererPool = RE_HtmlRendererPool;
    })(rm_renderingservices = exports.rm_renderingservices || (exports.rm_renderingservices = {}));
});
//# sourceMappingURL=RE_HtmlRendererPool.js.map