...
(function (rm_coreservices) {
    var RE_PlaylistItem_Status = (function (_super) {
        ...
        RE_PlaylistItem_Status.prototype.formatDate = function (date) {
            var year = date.getFullYear(), month = date.getMonth() + 1, day = date.getDate(), hour = date.getHours(), minute = date.getMinutes(), second = date.getSeconds(), hourFormatted = hour % 12 || 12, minuteFormatted = minute < 10 ? "0" + minute : minute, secondsFormatted = second < 10 ? "0" + second : second, morning = hour < 12 ? "am" : "pm";
        ...
        RE_PlaylistItem_Status.prototype.trace_01 = function (traceId, selectId, prefixMessage, sufixMessage) {
            var str1 = this.formatDate(new Date()) + " -  " + prefixMessage + " -  "
                + " " + this.rpad(this.getDiagram().getOwner().getPlaylistItemTypeName(), " ", 20)
                + " " + this.getDiagram().getOwner().getLogic().getOwnerIdxAsChild()
                + " " + this.getDiagram().getOwner().getPlaylistItemId()
