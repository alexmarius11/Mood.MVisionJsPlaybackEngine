define(["require", "exports", "./rsa", "../other/errors", "../bignum/bignum", "../other/get-random-values"], function (require, exports, rsa_1, errors_1, bignum_1, get_random_values_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var RSA_OAEP = /** @class */ (function () {
        function RSA_OAEP(key, hash, label) {
            this.rsa = new rsa_1.RSA(key);
            this.hash = hash;
            if (label !== undefined) {
        ...
        RSA_OAEP.prototype.encrypt = function (data, random) {
            ...
            if (data_length > key_size - 2 * this.hash.HASH_SIZE - 2)
                throw new errors_1.IllegalArgumentError('data too large');
            ...
            if (random !== undefined) {
                if (seed.length !== random.length)
                    throw new errors_1.IllegalArgumentError('random size must equal the hash size');
                seed.set(random);
            }
            else {
                get_random_values_1.getRandomValues(seed);
            }
            ...
            for (var i = 0; i < seed.length; i++)
                seed[i] ^= seed_mask[i];
            this.rsa.encrypt(new bignum_1.BigNumber(message));
            return new Uint8Array(this.rsa.result);
        };
        RSA_OAEP.prototype.decrypt = function (data) {
            if (!this.rsa.key)
                throw new errors_1.IllegalStateError('no key is associated with the instance');
            var key_size = Math.ceil(this.rsa.key[0].bitLength / 8);
            var hash_size = this.hash.HASH_SIZE;
            var data_length = data.byteLength || data.length || 0;
            if (data_length !== key_size)
                throw new errors_1.IllegalArgumentError('bad data');
            this.rsa.decrypt(new bignum_1.BigNumber(data));
            ...
            if (z !== 0)
                throw new errors_1.SecurityError('decryption failed');
            var seed_mask = this.RSA_MGF1_generate(data_block, seed.length);
            for (var i = 0; i < seed.length; i++)
                seed[i] ^= seed_mask[i];
            var data_block_mask = this.RSA_MGF1_generate(seed, data_block.length);
    ...
    var RSA_PSS = /** @class */ (function () {
        ...
        RSA_PSS.prototype.sign = function (data, random) {
            ...
            if (salt_length > 0) {
                if (random !== undefined) {
                    if (m_salt.length !== random.length)
                        throw new errors_1.IllegalArgumentError('random size must equal the salt size');
                    m_salt.set(random);
                }
                else {
                    get_random_values_1.getRandomValues(m_salt);
                }
            }
            d_block[ps_length] = 1;
            d_salt.set(m_salt);
            ...
            this.rsa.decrypt(new bignum_1.BigNumber(message));
        ...
        RSA_PSS.prototype.verify = function (signature, data) {
            ...
            var salt_length = this.saltLength;
            var ps_length = message_length - salt_length - hash_size - 2;
            this.rsa.encrypt(new bignum_1.BigNumber(signature));
            var message = this.rsa.result;
            if (message[message_length - 1] !== 0xbc)
                throw new errors_1.SecurityError('bad signature');
            var h_block = message.subarray(message_length - hash_size - 1, message_length - 1);
            var d_block = message.subarray(0, message_length - hash_size - 1);
            var d_salt = d_block.subarray(ps_length + 1);
            var zbits = 8 * message_length - key_bits + 1;
    ...
    var RSA_PKCS1_v1_5 = /** @class */ (function () {
        ...
        RSA_PKCS1_v1_5.prototype.sign = function (data) {
            ...
            this.rsa.decrypt(new bignum_1.BigNumber(em));
            return this.rsa.result;
        };
